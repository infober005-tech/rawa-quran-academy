...
function SettingsPage() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (error) toast.error(error.message); else { toast.success(t("common.saved")); await refresh(); }
  };

  const changePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (pw.next.length < 8) return toast.error(t("common.password_min"));
    if (pw.next !== pw.confirm) return toast.error(t("common.password_mismatch"));
    setPwBusy(true);
    const { error } = await supabase.auth.updateUser({ password: pw.next });
    setPwBusy(false);
    if (error) toast.error(error.message);
    else { toast.success(t("common.saved")); setPw({ next: "", confirm: "" }); }
  };
        ...
        <div className="grid lg:grid-cols-2 gap-6 max-w-5xl">
          <section className="rounded-3xl bg-card/70 backdrop-blur-xl border border-border shadow-soft p-6 space-y-4">
            <header className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-primary">{t("settings.language.title")}</h2>
            </header>
            <LangSwitcher />
          </section>
          ...
          <form onSubmit={changePassword} aria-label="Change password" className="rounded-3xl bg-card/70 backdrop-blur-xl border border-border shadow-soft p-6 space-y-4">
            ...
            <Field label={t("settings.new_password")} id="s-pw1"><input id="s-pw1" type="password" value={pw.next} onChange={(e) => setPw((p) => ({ ...p, next: e.target.value }))} className={input} autoComplete="new-password" /></Field>
            <Field label={t("settings.confirm_password")} id="s-pw2"><input id="s-pw2" type="password" value={pw.confirm} onChange={(e) => setPw((p) => ({ ...p, confirm: e.target.value }))} className={input} autoComplete="new-password" /></Field>
